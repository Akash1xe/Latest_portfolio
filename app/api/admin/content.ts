...
import { ensureSettingsTable, getDb, parseStoredJson } from "../_db.js";
...
const contentSchema = z.object({
  ...
  about: z.object({
    background: z
      .array(
        z.object({ question: nonEmpty, answer: z.string().min(1).max(2000) })
      )
      .max(20),
  ...
  skills: z
    .array(z.object({ category: nonEmpty, skills: z.array(nonEmpty).max(50) }))
    .max(20),
...
export default async function handler(
  ...
  if (!db) {
    if (request.method === "GET")
      return response
        .status(200)
        .json({ content: defaultPortfolioContent, storage: "unconfigured" });
    return response
      .status(503)
      .json({ error: "Set DATABASE_URL to enable admin persistence." });
  }
  try {
    ...
    if (request.method === "GET") {
      ...
      const saved = parseStoredJson<Partial<PortfolioContent>>(
        portfolioRows[0]?.content
      );
      const legacy = parseStoredJson<Record<string, unknown>>(
      ...
      const content = {
        ...
        competitiveProgramming: {
          ...defaultPortfolioContent.competitiveProgramming,
          ...legacyCp,
          ...saved?.competitiveProgramming,
        },
      };
      return response
        .status(200)
        .json({
          content,
          updatedAt: portfolioRows[0]?.updated_at ?? null,
          storage: "postgres",
        });
    }

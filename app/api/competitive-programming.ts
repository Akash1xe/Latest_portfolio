...
import { competitiveProgrammingFallback } from "../src/features/competitive-programming/fallback";
import type {
  CalendarDay,
  CompetitiveProgrammingData,
  ContestPoint,
  RecentProblem,
} from "../src/features/competitive-programming/types";
import { requireAdmin } from "./_auth";
import { parseStoredJson } from "./_db";
...
const loadSettings = async (): Promise<IntegrationSettings> => {
  ...
  try {
    const rows = await db`select id, content from portfolio_settings where id in ('portfolio-content', 'competitive-programming')`;
    const portfolio = parseStoredJson<{ competitiveProgramming?: Partial<IntegrationSettings>; publishState?: "draft" | "published" }>(
    ...
    return {
      ...
      publishState: portfolio?.publishState ?? legacy?.publishState ?? defaults.publishState,
    };
  } catch {
    return defaults;
  } finally {
...
const calculateStreaks = (calendar: CalendarDay[]) => {
  const active = new Set(calendar.filter((day) => day.count > 0).map((day) => day.date));
...
const fetchLeetCode = async (username: string) => {
  ...
  if (!payload.data?.matchedUser) throw new Error(payload.errors?.[0]?.message ?? "LeetCode user not found");

  const stats = payload.data.matchedUser.submitStats?.acSubmissionNum ?? [];
  const countFor = (difficulty: string) =>
    stats.find((entry) => entry.difficulty.toLowerCase() === difficulty.toLowerCase())?.count ?? 0;
  const rawCalendar = JSON.parse(payload.data.matchedUser.submissionCalendar || "{}") as Record<string, number>;
...
const codeforcesRequest = async <T,>(method: string, params: URLSearchParams) => {
  ...
  const payload = (await response.json()) as { status: string; result?: T; comment?: string };
  if (payload.status !== "OK" || !payload.result) throw new Error(payload.comment ?? "Codeforces request failed");
  return payload.result;
};

const fetchCodeforces = async (username: string) => {
  type User = { rating?: number; maxRating?: number; rank?: string };
  type Rating = { contestName: string; newRating: number; rank: number; ratingUpdateTimeSeconds: number };
  type Submission = {
    id: number;
    verdict?: string;
  ...
  const users = await codeforcesRequest<User[]>("user.info", new URLSearchParams({ handles: username }));
  await sleep(CODEFORCES_DELAY);
  const ratingHistory = await codeforcesRequest<Rating[]>("user.rating", new URLSearchParams({ handle: username }));
  await sleep(CODEFORCES_DELAY);
  ...
  solved.forEach((submission) =>
    submission.problem.tags.forEach((tag) => tagCount.set(tag, (tagCount.get(tag) ?? 0) + 1))
  );
  const solvedByRating = { "800–1000": 0, "1100–1300": 0, "1400+": 0, unrated: 0 };
  solved.forEach((submission) => {
  ...
  return {
    ...
    contests: ratingHistory.map(
      (contest): ContestPoint => ({
    ...
    tags: [...tagCount.entries()]
      ...
      .sort((a, b) => b.count - a.count)
      .slice(0, 10),
    recentAccepted: accepted.slice(0, 10).map(
      (submission): RecentProblem => ({
...
const buildStats = async (): Promise<CompetitiveProgrammingData> => {
  ...
  const leetcode =
    leetcodeResult.status === "fulfilled"
      ? { ...competitiveProgrammingFallback.leetcode, username: leetcodeUsername, profileUrl: `https://leetcode.com/u/${leetcodeUsername}/`, ...leetcodeResult.value }
      : { ...competitiveProgrammingFallback.leetcode, username: leetcodeUsername, profileUrl: `https://leetcode.com/u/${leetcodeUsername}/` };
  const codeforces =
    codeforcesResult.status === "fulfilled"
      ? { ...competitiveProgrammingFallback.codeforces, username: codeforcesUsername, profileUrl: `https://codeforces.com/profile/${codeforcesUsername}`, ...codeforcesResult.value }
      : { ...competitiveProgrammingFallback.codeforces, username: codeforcesUsername, profileUrl: `https://codeforces.com/profile/${codeforcesUsername}` };
  const now = new Date();
  const bothLive = leetcodeResult.status === "fulfilled" && codeforcesResult.status === "fulfilled";
  const bothFailed = leetcodeResult.status === "rejected" && codeforcesResult.status === "rejected";
  ...
  return {
    profile: {
      ...
      totalSolved: Math.max(leetcode.totalSolved + codeforces.totalSolved, settings.fallbackTotal),
    },
    leetcode,
    codeforces,
    services: {
      leetcode: leetcodeResult.status === "fulfilled" ? "connected" : "fallback",
      codeforces: codeforcesResult.status === "fulfilled" ? "connected" : "fallback",
      cache: "healthy",
    },
    meta: {
...
export default async function handler(request: VercelRequest, response: VercelResponse) {
  ...
  if (!forceRefresh && cacheHost.cpStatsCacheV2 && cacheHost.cpStatsCacheV2.expiresAt > Date.now()) {
    response.setHeader("x-stats-cache", "HIT");
    response.setHeader("cache-control", "public, s-maxage=10800, stale-while-revalidate=86400");

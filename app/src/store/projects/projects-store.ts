...
const useProjectStore = create<ProjectState>((set, get) => ({
  ...
  fetchProjects: async () => {
    ...
    try {
      ...
      const staticProjects: Project[] = await response.json();
      const payload = contentResponse.ok
        ? await contentResponse.json() as { content?: { projects?: Project[] } }
        : null;
      const projects = payload?.content?.projects?.length
        ? payload.content.projects
        : staticProjects;
